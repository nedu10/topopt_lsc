...
int solve(DataObj data) {
    ...
    if (opt->localVolumeStatus) {
        local = new LocalVolume(opt->da_nodes, opt->xPhys, opt->xPassive, opt->gradedPorousAlpha, data);
    }
    ...
    while (itr < opt->maxItr && ch > opt->tol) {
        ...
        if (opt->localVolumeStatus) {
            ierr = local->Constraint(opt->xPhys, &(opt->gx[1]), opt->dgdx[1], opt->gradedPorousAlpha);
            CHKERRQ(ierr);
        }
