...
public:
    // Constructor
    LocalVolume(DM da_nodes, Vec x, Vec xPassive, Vec gradedPorousAlpha, DataObj data);

    // Destructor
    ~LocalVolume();

    // LocalVolume design variables
    PetscErrorCode Constraint(Vec x, PetscScalar *gx, Vec dx, Vec gradedPorousAlpha);
...
private:
    ...
    PetscErrorCode SetUp(DM da_nodes, Vec xPassive);
